...
export default function Dashboard() {
    ...
    async function fetchData() {
        try {
            ...
            const [healthRes, eventsRes] = await Promise.all([
                supabase.from('daily_health_scores')
    ...
    const scoreColor = score >= 75 ? '#2ecc71' : score >= 50 ? '#f39c12' : '#e74c3c'
                    ...
                    <table className="w-full text-sm">
                        <thead>
                            <tr className="border-b border-gray-800">
                                {['Time', 'Source', 'Amount', 'Severity', 'Z-Score', 'AI Explanation'].map(h => (
                            ...
                            {events.length === 0 ? (
                                ...
                                events.map((e, i) => (
                                    <tr key={i} className="border-b border-gray-800 hover:bg-gray-800 transition-colors">
                                        ...
                                        <td className="px-4 py-3 text-white capitalize">{e.source}</td>
